...
class Squeeze(suite.Suite):
    updategrub = "/usr/sbin/update-grub"
    grubroot = "/usr/lib/grub"
    valid_flavours = { 'i386' :  ['486', '686'],
                       'amd64' : ['amd64']}
    default_flavour = { 'i386' : '686', 'amd64' : 'amd64' }
# find out the correct ids for amd64
    ec2_kernel_info = { 'i386' : 'aki-5f15f636', 'amd64' : 'aki-4f4daa26' }
    ec2_ramdisk_info = { 'i386': 'ari-d5709dbc', 'amd64' : 'ari-4c4daa25' }
    disk_prefix = 'hd'
    ...
    def install_fstab(self):
        if self.vm.hypervisor.preferred_storage == VMBuilder.hypervisor.STORAGE_FS_IMAGE:
            self.install_from_template('/etc/fstab', 'squeeze_fstab_fsimage', { 'fss' : disk.get_ordered_filesystems(self.vm), 'prefix' : self.disk_prefix })
        else:
            self.install_from_template('/etc/fstab', 'squeeze_fstab', { 'parts' : disk.get_ordered_partitions(self.vm.disks), 'prefix' : self.disk_prefix })
