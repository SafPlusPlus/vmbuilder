...
class Debian(Distro):
    ...
    def register_options(self):
        ...
        group.add_option('--suite', default='squeeze', help='Suite to install. Valid options: %s [default: %%default]' % ' '.join(self.suites))
        ...
        group.add_option('--mirror', metavar='URL', help='Use Debian mirror at URL instead of the default, which is http://ftp.us.debian.org/debian for official arches.')
        ...
        group.add_option('--security-mirror', metavar='URL', help='Use Debian security mirror at URL instead of the default, which is http://security.debian.org for official arches.')
    ...
    def set_defaults(self):
        if not self.vm.mirror:
            self.vm.mirror = 'http://ftp.us.debian.org/debian/'

        if not self.vm.security_mirror:
            self.vm.security_mirror = 'http://security.debian.org/'
    ...
    def preflight_check(self):
        ...
        if self.vm.hypervisor.name == 'Xen':
            logging.info('Xen kernel default: linux-image-2.6-%s %s', self.suite.xen_kernel_flavour, self.xen_kernel_version())
